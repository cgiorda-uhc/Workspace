...
namespace AddCoverLetterToExistingPDF_PRSpecNov2017
{
    class AddCoverLetterToExistingPDF_PRSpecNov2017
    {
        ...
        private static void prepFile(string strFile)
        {
            ...
            if (dt == null)
            {
                ...
                //string strSQL = "SELECT a.MPIN, 'October 1, 2017' as [Date], FirstName + ' ' + LastName as [Physician Name], ProvDegree as [Prov_Degree], Spec_display as Specialty, b.Street as [Address 1], b.City, b.[State], b.zipcd as [ZIP Code], P_LastName, RCMO, RCMO_title as [RCMO title] FROM dbo.PBP_Outl_ph12 as a inner join dbo.PBP_outl_demogr_ph12 as b on a.MPIN=b.MPIN inner join dbo.PBP_outl_TIN_addr_ph12 as ad on ad.TaxID=b.TaxID inner join dbo.PBP_spec_handl_ph12 as h on h.MPIN=a.mpin inner join dbo.PBP_dim_RCMO as r on r.Region=b.RGN_NM WHERE a.Exclude in(0,5) AND r.phase_id=2";


                //string strSQL = "select a.MPIN, 'June ???, 2018' as [Date], FirstName + ' ' + LastName as [Physician Name], ProvDegree as [Prov_Degree], Spec_display as Specialty, b.Street as [Address 1], b.City, b.[State], b.zipcd as [ZIP Code], P_LastName, RCMO, RCMO_title as [RCMO title] from dbo.PBP_Outl_Ph32 as a inner join dbo.PBP_outl_demogr_ph32 as b on a.MPIN=b.MPIN inner join dbo.PBP_outl_PTI_addr_ph32 as p on p.mpin=PTIGroupID_upd inner join dbo.PBP_spec_handl_ph32 as h on h.MPIN=a.mpin inner join dbo.PBP_dim_RCMO as r on r.Region=b.RGN_NM where a.Exclude in(0,5) and r.phase_id=2 AND ADDTIONAL_FILTERS=INNA_HELP ";


                string strSQL = "select a.MPIN, 'June 15, 2018' as [Date], FirstName + ' ' + LastName as [Physician Name], ProvDegree as [Prov_Degree], b.Street as [Address 1], b.City, b.[State], b.zipcd as [ZIP Code], P_LastName, RCMO, RCMO_title  from dbo.PBP_Outl_Ph32 as a inner join dbo.PBP_outl_demogr_ph32 as b on a.MPIN=b.MPIN inner join dbo.PBP_outl_PTI_addr_ph32 as p on p.mpin=PTIGroupID_upd inner join dbo.PBP_spec_handl_ph32 as h on h.MPIN=a.mpin inner join dbo.PBP_dim_RCMO as r on r.Region=b.RGN_NM where a.Exclude in(0,5) and r.phase_id=2 ";
            ...
            if (drTmp.Length > 0)
            {
                ...
                //sb.Append(strCurrentMPIN_GLOBAL + ",");
            }
            else
                return;
            ...
            PDFHelper.mergePDF(strTempPath_GLOBAL + "temp.pdf", strFile, strNewFullFilePath, true);
