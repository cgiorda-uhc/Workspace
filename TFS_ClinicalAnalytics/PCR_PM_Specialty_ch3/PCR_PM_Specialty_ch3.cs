...
namespace PCR_PM_Specialty_ch3
{
    class PCR_PM_Specialty_ch3
    {
        static void Main(string[] args)
        {
            ...
            try
            {
                ...
                string strSampleCount = "7";
                string strTinList = "SELECT MPIN FROM (SELECT TOP " + strSampleCount + "  ad.MPIN FROM dbo.PBP_Outl_Ph33 as a inner join dbo.PBP_outl_demogr_Ph33 as b on a.MPIN=b.MPIN inner join dbo.PBP_outl_PTI_addr_Ph33 as ad on ad.mpin=b.PTIGroupID_upd inner join dbo.PBP_spec_handl_Ph33 as h on h.MPIN=a.mpin inner join dbo.PBP_dim_RCMO as r on ad.RGN_NM=r.Region WHERE a.Exclude in(0,5) AND b.PTIGroupID>0 AND r.phase_id=2 group by ad.MPIN HAVING SUM(Tot_Util_meas) > 0 AND SUM(Tot_PX_meas) > 0 ORDER BY NEWID() ) as a UNION SELECT MPIN FROM (SELECT TOP " + strSampleCount + "  ad.MPIN FROM dbo.PBP_Outl_Ph33 as a inner join dbo.PBP_outl_demogr_Ph33 as b on a.MPIN=b.MPIN inner join dbo.PBP_outl_PTI_addr_Ph33 as ad on ad.mpin=b.PTIGroupID_upd inner join dbo.PBP_spec_handl_Ph33 as h on h.MPIN=a.mpin inner join dbo.PBP_dim_RCMO as r on ad.RGN_NM=r.Region WHERE a.Exclude in(0,5) AND b.PTIGroupID>0 AND r.phase_id=2 group by ad.MPIN HAVING SUM(Tot_Util_meas) <= 0 AND SUM(Tot_PX_meas) > 0 ORDER BY NEWID() ) as b UNION SELECT MPIN FROM (SELECT TOP " + strSampleCount + "  ad.MPIN FROM dbo.PBP_Outl_Ph33 as a inner join dbo.PBP_outl_demogr_Ph33 as b on a.MPIN=b.MPIN inner join dbo.PBP_outl_PTI_addr_Ph33 as ad on ad.mpin=b.PTIGroupID_upd inner join dbo.PBP_spec_handl_Ph33 as h on h.MPIN=a.mpin inner join dbo.PBP_dim_RCMO as r on ad.RGN_NM=r.Region WHERE a.Exclude in(0,5) AND b.PTIGroupID>0 AND r.phase_id=2 group by ad.MPIN HAVING SUM(Tot_Util_meas) > 0 AND SUM(Tot_PX_meas) <= 0 ORDER BY NEWID() ) as c UNION SELECT MPIN FROM (SELECT TOP " + strSampleCount + "  ad.MPIN FROM dbo.PBP_Outl_Ph33 as a inner join dbo.PBP_outl_demogr_Ph33 as b on a.MPIN=b.MPIN inner join dbo.PBP_outl_PTI_addr_Ph33 as ad on ad.mpin=b.PTIGroupID_upd inner join dbo.PBP_spec_handl_Ph33 as h on h.MPIN=a.mpin inner join dbo.PBP_dim_RCMO as r on ad.RGN_NM=r.Region WHERE a.Exclude in(0,5) AND b.PTIGroupID>0 AND r.phase_id=2 group by ad.MPIN HAVING COUNT(ad.TaxID) > 15 ORDER BY NEWID()) as d ";
                ...
                else
                {
                   ...
                   // strSQL = "select ad.TaxID,ad.MPIN as PracticeId,ad.Practice_Name,ad.Street,ad.City,ad.State,ad.ZipCd, RCMO,RCMO_title,RCMO_title1, NULL AS Folder_Name from dbo.PBP_Outl_Ph33 as a inner join dbo.PBP_outl_demogr_Ph33 as b on a.MPIN=b.MPIN inner join dbo.PBP_outl_PTI_addr_Ph33 as ad on ad.mpin=b.PTIGroupID_upd inner join dbo.PBP_spec_handl_Ph33 as h on h.MPIN=a.mpin inner join dbo.PBP_dim_RCMO as r on ad.RGN_NM=r.Region where a.Exclude in(0,5) and b.PTIGroupID>0 and r.phase_id=2 and ad.MPIN in (" + strTinList + ") group by ad.TaxID,ad.MPIN,ad.Practice_Name,ad.Street,ad.City,ad.State,ad.ZipCd, RCMO,RCMO_title,RCMO_title1"; //ADD  a.ndb_specialty????????????????????????????????????????????????????????????????????????????????????????????


                    strSQL = "SELECT ad.TaxID, ad.MPIN as PracticeId, ad.Practice_Name, ad.Street, ad.City, ad.State, ad.ZipCd, RCMO, RCMO_title, RCMO_title1, (SELECT SUM(p.outl_idx) + SUM(p.outl_idx_g) as all_util FROM dbo.PBP_Profile_ph33 as p inner join dbo.PBP_Outl_ph33 as o on o.mpin=p.mpin inner join dbo.PBP_outl_demogr_ph33 as d on o.MPIN=d.MPIN WHERE attr_clients>=20 AND o.exclude in(0,5) AND d.PTIGroupID=ad.MPIN) as Tot_Util_meas, (SELECT SUM(Outl_idx)+SUM(Outl_idx_g) as all_proc FROM dbo.PBP_Profile_px_ph33 as p inner join dbo.PBP_outl_ph33 as o on o.MPIN=p.MPIN inner join dbo.PBP_outl_demogr_ph33 as d on o.MPIN=d.MPIN WHERE Exclude in(0,5) AND Measure_ID not in(40,41,42,44) AND Outl_idx is not null AND d.PTIGroupID=ad.MPIN) as Tot_PX_meas, NULL AS Folder_Name FROM dbo.PBP_Outl_Ph33 as a inner join dbo.PBP_outl_demogr_Ph33 as b on a.MPIN=b.MPIN inner join dbo.PBP_outl_PTI_addr_Ph33 as ad on ad.mpin=b.PTIGroupID_upd inner join dbo.PBP_spec_handl_Ph33 as h on h.MPIN=a.mpin inner join dbo.PBP_dim_RCMO as r on ad.RGN_NM=r.Region WHERE a.Exclude in(0,5) AND b.PTIGroupID>0 AND r.phase_id=2 AND ad.MPIN in (" + strTinList + ") GROUP BY ad.TaxID, ad.MPIN, ad.Practice_Name, ad.Street, ad.City, ad.State, ad.ZipCd, RCMO, RCMO_title, RCMO_title1";

                    strSQL = "SELECT ad.TaxID, ad.MPIN as PracticeId, ad.Practice_Name, ad.Street, ad.City, ad.State, ad.ZipCd, RCMO, RCMO_title, RCMO_title1, (SELECT SUM(p.outl_idx) + SUM(p.outl_idx_g) as all_util FROM dbo.PBP_Profile_ph33 as p inner join dbo.PBP_Outl_ph33 as o on o.mpin=p.mpin inner join dbo.PBP_outl_demogr_ph33 as d on o.MPIN=d.MPIN WHERE attr_clients>=20 AND o.exclude in(0,5) AND d.PTIGroupID=ad.MPIN) as Tot_Util_meas, (SELECT SUM(Outl_idx)+SUM(Outl_idx_g) as all_proc FROM dbo.PBP_Profile_px_ph33 as p inner join dbo.PBP_outl_ph33 as o on o.MPIN=p.MPIN inner join dbo.PBP_outl_demogr_ph33 as d on o.MPIN=d.MPIN WHERE Exclude in(0,5) AND Measure_ID not in(40,41,42,44) AND Outl_idx is not null AND d.PTIGroupID=ad.MPIN) as Tot_PX_meas, NULL AS Folder_Name FROM dbo.PBP_Outl_Ph33 as a inner join dbo.PBP_outl_demogr_Ph33 as b on a.MPIN=b.MPIN inner join dbo.PBP_outl_PTI_addr_Ph33 as ad on ad.mpin=b.PTIGroupID_upd inner join dbo.PBP_spec_handl_Ph33 as h on h.MPIN=a.mpin inner join dbo.PBP_dim_RCMO as r on ad.RGN_NM=r.Region WHERE a.Exclude in(0,5) AND b.PTIGroupID>0 AND r.phase_id=2 GROUP BY ad.TaxID, ad.MPIN, ad.Practice_Name, ad.Street, ad.City, ad.State, ad.ZipCd, RCMO, RCMO_title, RCMO_title1";
                ...
                foreach (DataRow dr in dtMain.Rows)//MAIN LOOP START
                {
                   ...
                   // strSQL = "SELECT SUM(p.outl_idx) + SUM(p.outl_idx_g) as all_util FROM dbo.PBP_Profile_ph33 as p inner join dbo.PBP_Outl_ph33 as o on o.mpin = p.mpin inner join dbo.PBP_outl_demogr_ph33 as d on o.MPIN = d.MPIN WHERE attr_clients >= 20 AND o.exclude in(0, 5) AND d.PTIGroupID = " + strPracticeID ;
                    ...
                    if (blHasWord)
                    {
                       ...
                       // MSWord.wordReplace("{$Provider_Specialty}", strSpecialtyProperCase);//strSpecialty????
                    ...
                    if (blHasUtilization)
                    {
                       ...
                       // strSQL = "select SUM(o.outl_idx) as tot_meas,SUM(o.outl_idx_g) as fav_meas from dbo.PBP_Profile_ph33 as p inner join dbo.PBP_Outl_ph33_models as o on o.mpin=p.mpin inner join dbo.PBP_outl_demogr_ph33 as d on o.MPIN=d.MPIN where clients>=20 and d.PTIGroupID=" + strPracticeID + " group by d.PTIGroupID,sort_ID,Measure_desc order by sort_ID ";

                        //strSQL = "SELECT SUM(outl_idx) as tot_meas, SUM(outl_idx_g) as fav_meas FROM dbo.PBP_Profile_ph33 as p inner join dbo.PBP_Outl_ph33 as o on o.mpin=p.mpin inner join dbo.PBP_outl_demogr_ph33 as d on o.MPIN=d.MPIN WHERE attr_clients>=20 AND o.exclude in(0,5) AND d.PTIGroupID=" + strPracticeID + " GROUP BY d.PTIGroupID, sort_ID, Measure_desc ORDER BY sort_ID";
