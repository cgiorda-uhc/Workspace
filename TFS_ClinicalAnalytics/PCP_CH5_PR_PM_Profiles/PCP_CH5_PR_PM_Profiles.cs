...
namespace PCP_CH5_PR_PM_Profiles
{
    class PCP_CH5_PR_PM_Profiles
    {
        static void Main(string[] args)
        {
            ...
            try
            {
                ...
                strTinList = "SELECT distinct ad.MPIN FROM ph15.outliers6 as a inner join ph15.UHN_JAN14_DEMOG as b on a.MPIN=b.MPIN inner join ph15.UHN_JAN14_PTI_DEMOG as ad on ad.mpin=b.PTIGroupID_upd inner join ph15.OUTL_MODELS6 as m on m.mpin=a.mpin WHERE a.Exclude in(0,5) AND b.PTIGroupID>0";
                ...
                foreach (DataRow dr in dtMain.Rows)//MAIN LOOP START
                {
                    ...
                    else
                    {
                       ...
                       // strSQL = "select d.MPIN as MPIN,CASE WHEN P_FirstName IS NOT NULL THEN 'Dr.'+' '+P_FirstName+' '+P_LastName ELSE P_LastName END as dr_info from dbo.PBP_outl_demogr_ph14 as d inner join dbo.PBP_outl_ph14 as o on o.MPIN=d.MPIN where Exclude in(0,5) and d.PTIGroupID=" + strPracticeID + " order by P_LastName";
                        ...
                        //strSQL = "select d.PTIGroupID, 'Dr.'||' '||trim(P_FirstName)||' '||trim(P_LastName) as dr_info from ph15.UHN_JAN14_DEMOG as d inner join ph15.outliers6 as o on o.MPIN=d.MPIN where Exclude in(0,5) and d.PTIGroupID=" + strPracticeID + " order by P_LastName";


                        strSQL = "select d.MPIN, TRIM((CASE WHEN P_FirstName IS NOT NULL THEN 'Dr. ' || trim(P_FirstName) || ' ' ELSE '' END ) ||trim(P_LastName)) as dr_info from ph15.UHN_JAN14_DEMOG as d inner join ph15.outliers6 as o on o.MPIN=d.MPIN where Exclude in(0,5) and d.PTIGroupID=" + strPracticeID + "order by P_LastName";
                    ...
                    if (blHasUtilization)
                    {
                        ...
                        strSQL = "select  SUM(p.Outl_idx) as tot_meas, SUM(p.Outl_idx_g) as fav_meas from ph15.Profile as p inner join ph15.outliers6 as o on o.MPIN=p.MPIN inner join ph15.UHN_JAN14_DEMOG as d on o.MPIN=d.MPIN inner join ph15.outl_models6 as m on m.mpin=o.mpin where Exclude in(0,5) and m.model_id<>5 and measure_id in (1,2,3,5,37,50) and d.PTIGroupID=" + strPracticeID + "  group by d.PTIGroupID,sort_ID,Measure_desc order by sort_ID";
                        ...
                        if (dt.Rows.Count > 0)
                        {
                            ...
                            if (blHasWord)
                            {
                                ...
                                MSWord.tryCount = 0;
                                MSWord.pasteExcelTableToWord("utilization_table", strSheetname, "A1:C8", MSExcel.xlsApp, MSExcel.xlsWB, MSExcel.xlsSheet, blAddBookmark: false);
                    ...
                    if (blHasPharmacy)
                    {
                        ...
                        //strSQL = "select Measure_desc, sum(m.Outl_idx) as tot_meas, sum(m.Outl_idx_g) as fav_meas from ph15.Profile_px as p inner join ph15.outliers6 as o on o.MPIN=p.MPIN inner join ph15.UHN_JAN14_DEMOG as d on o.MPIN=d.MPIN inner join ph15.outl_models6 as m on m.mpin=o.mpin where Exclude in(0,5) and m.model_id<>4 and Measure_ID in(38,51,55) and d.PTIGroupID=" + strPracticeID + " group by d.PTIGroupID,sort_ID,Measure_desc order by sort_ID";


                        strSQL = "select Measure_desc, sum(p.Outl_idx) as tot_meas, sum(p.Outl_idx_g) as fav_meas from ph15.Profile_px as p inner join ph15.outliers6 as o on o.MPIN=p.MPIN inner join ph15.UHN_JAN14_DEMOG as d on o.MPIN=d.MPIN inner join ph15.outl_models61 as m on m.mpin=o.mpin where Exclude in(0,5) and m.model_id<>4 and Measure_ID in(38,51,55) and d.PTIGroupID = " + strPracticeID + " group by d.PTIGroupID,sort_ID,Measure_desc order by sort_ID";
