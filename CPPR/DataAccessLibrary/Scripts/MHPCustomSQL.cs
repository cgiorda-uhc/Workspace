...
namespace DataAccessLibrary.Scripts
{
    public static class MHPCustomSQL
    {
        ...
        public static string VolatileNameSF { get; set; } = "UGP_PRD_P01_DB.UGAP_TEMP_USERS.MissingMembersTmp";

        public static string VolatileName { get; set; } = "MissingMembersTmp";

        public static string VolatileColumnsDeclare { get; set; } = "mhp_uni_id BIGINT, Cardholder_ID_CLN  VARCHAR(11), State_Of_Issue VARCHAR(5),BTH_DT DATE, REQ_DT DATE, MBR_FST_NM VARCHAR(255), MBR_LST_NM VARCHAR(255) ";
        ...
        public static List<MHPParameterModel> MHPParameters_SF()
        {
            ...
            //EI
            //EI
            pm.Add(new MHPParameterModel() { MHPSQL = "AND (Classification = '" + LOS.EI + "' ) ", UGAPSQL = "inner join " + VolatileNameSF + " as mm on ltrim(a.MBR_ALT_ID, '0') = mm.Cardholder_ID_CLN AND a.BTH_DT = mm.BTH_DT AND mm.REQ_DT BETWEEN c.eff_dt AND LAST_DAY(c.eff_dt)", LOS = LOS.EI, SearchMethod = "MBR_ALT_ID/BD/RD" });
            pm.Add(new MHPParameterModel() { MHPSQL = "AND Classification = '" + LOS.EI + "'  AND [Enrollee_First_Name] IS NOT NULL AND [Enrollee_Last_Name] IS NOT NULL  AND [Member_Date_of_Birth]  IS NOT NULL ", UGAPSQL = "inner join " + VolatileNameSF + " as mm on upper(a.MBR_FST_NM) = upper(mm.MBR_FST_NM) AND upper(a.MBR_LST_NM) = upper(mm.MBR_LST_NM) AND a.BTH_DT = mm.BTH_DT AND mm.REQ_DT BETWEEN c.eff_dt AND LAST_DAY(c.eff_dt) ", LOS = LOS.EI, SearchMethod = "FN/LN/BD/RD" });
            pm.Add(new MHPParameterModel() { MHPSQL = "AND Classification = '" + LOS.EI + "' AND [Enrollee_First_Name] IS NOT NULL AND [Enrollee_Last_Name] IS NOT NULL  AND [Member_Date_of_Birth]  IS NOT NULL ", UGAPSQL = "inner join " + VolatileNameSF + " as mm on upper(a.MBR_FST_NM) LIKE upper(mm.MBR_FST_NM) AND  upper(a.MBR_LST_NM) LIKE upper(mm.MBR_LST_NM) AND a.BTH_DT = mm.BTH_DT AND mm.REQ_DT BETWEEN c.eff_dt AND LAST_DAY(c.eff_dt) ", LOS = LOS.EI, SearchMethod = "FN%3/LN%3/BD/RD" });
            //EI_OX
            //EI_OX
            //EI_OX
            pm.Add(new MHPParameterModel() { MHPSQL = "AND (Classification = '" + LOS.EI_OX + "' )  ", UGAPSQL = "inner join " + VolatileNameSF + " as mm on ltrim(a.MBR_ID, '0') = mm.Cardholder_ID_CLN AND a.BTH_DT = mm.BTH_DT AND mm.REQ_DT BETWEEN c.eff_dt AND LAST_DAY(c.eff_dt) ", LOS = LOS.EI_OX, SearchMethod = "MBR_ID/BD/RD" });
            ...
            //IFP
            //IFP
            pm.Add(new MHPParameterModel() { MHPSQL = "AND (Classification = '" + LOS.IFP + "' ) ", UGAPSQL = "inner join " + VolatileNameSF + " as mm on SUBSTR(a.MBR_ID, 0,10) = mm.Cardholder_ID_CLN AND a.BTH_DT = mm.BTH_DT AND mm.REQ_DT BETWEEN c.eff_dt AND LAST_DAY(c.eff_dt) ", LOS = LOS.IFP, SearchMethod = "MBR_ID/BD/RD" });
            ...
            //CS
            //CS
            pm.Add(new MHPParameterModel() { MHPSQL = "AND Classification = '" + LOS.CS + "' AND [Member_Date_of_Birth]  IS NOT NULL ", UGAPSQL = "inner join " + VolatileNameSF + " as mm on ltrim(a.MBR_ID, '0') = mm.Cardholder_ID_CLN  AND k.CS_CO_CD_ST = mm.State_Of_Issue AND a.BTH_DT = mm.BTH_DT AND mm.REQ_DT BETWEEN c.eff_dt AND LAST_DAY(c.eff_dt) ", LOS = LOS.CS, SearchMethod = "MBR_ID/CS_CO_CD_ST/BD/RD" });
            pm.Add(new MHPParameterModel() { MHPSQL = "AND Classification = '" + LOS.CS + "' AND [Member_Date_of_Birth]  IS NOT NULL  ", UGAPSQL = "inner join " + VolatileNameSF + " as mm on ltrim(a.SBSCR_MEDCD_RCIP_NBR, '0')  = mm.Cardholder_ID_CLN  AND k.CS_CO_CD_ST = mm.State_Of_Issue AND a.BTH_DT = mm.BTH_DT AND mm.REQ_DT BETWEEN c.eff_dt AND LAST_DAY(c.eff_dt) ", LOS = LOS.CS, SearchMethod = "SBSCR_MEDCD_RCIP_NBR/CS_CO_CD_ST/BD/RD" });
        ...
        public static string UGAPSQLMemberDataCS_SF(string strFilterJoin, bool blIsCS)
        {
            ...
            sbSQL.Append("create or replace table " + VolatileNameSF + "( ");

            sbSQL.Append(VolatileColumnsDeclare);

            sbSQL.Append(");");

            sbSQL.Append("INSERT INTO " + VolatileNameSF + "(" + VolatileColumns + ") ");
            ...
            sbSQL.Append("FROM UGP_PRD_P01_DB.uhcdm001.hp_member a  ");
            sbSQL.Append("join UGP_PRD_P01_DB.uhcdm001.cs_demographics b on a.MBR_SYS_ID = b.MBR_SYS_ID  ");
            sbSQL.Append("join UGP_PRD_P01_DB.uhcdm001.date_eff c on b.DT_SYS_ID = c.EFF_DT_SYS_ID  ");
            sbSQL.Append("join UGP_PRD_P01_DB.uhcdm001.SOURCE_SYSTEM_COMBO d on b.SRC_SYS_COMBO_SYS_ID = d.SRC_SYS_COMBO_SYS_ID  ");
            sbSQL.Append("join UGP_PRD_P01_DB.uhcdm001.SUBSCRIBER_COUNTY h on b.SBSCR_CNTY_SYS_ID = h.SBSCR_CNTY_SYS_ID  ");
            sbSQL.Append("join UGP_PRD_P01_DB.uhcdm001.PRODUCT i on b.PRDCT_SYS_ID = i.PRDCT_SYS_ID  ");
            sbSQL.Append("join UGP_PRD_P01_DB.uhcdm001.cs_company_code k on b.CS_CO_CD_SYS_ID = k.CS_CO_CD_SYS_ID ");
        ...
        public static string UGAPSQLLMemberDataEI_SF(string strFilterJoin, bool blIsOX)
        {
            ...
            sbSQL.Append("a.SBSCR_MEDCD_RCIP_NBR ");
            sbSQL.Append("FROM UGP_PRD_P01_DB.uhcdm001.hp_member a  ");
            sbSQL.Append("join UGP_PRD_P01_DB.uhcdm001.hp_demographics b on a.MBR_SYS_ID = b.MBR_SYS_ID  ");
            sbSQL.Append("join UGP_PRD_P01_DB.uhcdm001.date_eff c on b.DT_SYS_ID = c.EFF_DT_SYS_ID  ");
            sbSQL.Append("join UGP_PRD_P01_DB.uhcdm001.SOURCE_SYSTEM_COMBO d on b.SRC_SYS_COMBO_SYS_ID = d.SRC_SYS_COMBO_SYS_ID  ");
            sbSQL.Append("join UGP_PRD_P01_DB.uhcdm001.SUBSCRIBER_COUNTY h on b.SBSCR_CNTY_SYS_ID = h.SBSCR_CNTY_SYS_ID  ");
            sbSQL.Append("join UGP_PRD_P01_DB.uhcdm001.LEGAL_ENTITY e on b.LEG_ENTY_SYS_ID = e.LEG_ENTY_SYS_ID  ");
            sbSQL.Append("join UGP_PRD_P01_DB.uhcdm001.MARKET_TYPE_CODE f on b.MKT_TYP_CD_SYS_ID = f.MKT_TYP_CD_SYS_ID  ");
            sbSQL.Append("join UGP_PRD_P01_DB.uhcdm001.CUSTOMER_SEGMENT g on b.CUST_SEG_SYS_ID = g.CUST_SEG_SYS_ID  ");
            sbSQL.Append("join UGP_PRD_P01_DB.uhcdm001.PRODUCT i on b.PRDCT_SYS_ID = i.PRDCT_SYS_ID  ");
            sbSQL.Append("join UGP_PRD_P01_DB.uhcdm001.GROUP_INDICATOR j on b.GRP_IND_SYS_ID = j.GRP_IND_SYS_ID  ");
            sbSQL.Append("join UGP_PRD_P01_DB.uhcdm001.company_code k on b.CO_CD_SYS_ID = k.CO_CD_SYS_ID  ");
            sbSQL.Append(strFilterJoin);
        ...
        public static string UGAPVolatileInsertSF(MHPMemberSearchModel mhp, MHPParameterModel mhp_param)
        {
            StringBuilder sbSQL = new StringBuilder();
            ...
            sbSQL.Append("SELECT " + id + ",'" + cidc + "','" + st + "', '" + bd + "', '" + rd + "', '" + fnw + "', '" + lnw + "' UNION ALL ");
