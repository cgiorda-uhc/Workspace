...
UPDATE Track
SET 
Track.[CODE_TYPE] = Chemo.[CODE_TYPE],
Track.[CODE_END_DT] = Chemo.[CODE_END_DT],
...
Track.[CODE_TYPE_PREVIOUS] = Chemo.[CODE_TYPE],
Track.[CODE_END_DT_PREVIOUS] = Chemo.[CODE_END_DT],
...
WHERE Track.[UPDATE_USER] = @username AND Track.UPDATE_DT =@update_date AND Track.[UPDATE_ACTION] = 'DELETE'
  ...
  --INSERT
INSERT INTO [chemopx].[ChemotherapyPX] ([CODE] ,[CODE_TYPE],[CODE_END_DT],[GENERIC_NAME] ,[TRADE_NAME] ,[CKPT_INHIB_IND] ,[ANTI_EMETIC_IND] ,[CODE_EFF_DT] ,[NHNR_CANCER_THERAPY] ,[CODE_CATEGORY_ID] ,[ASP_CATEGORY_ID] ,[DRUG_ADM_MODE_ID] ,[PA_DRUGS_ID] ,[PA_EFF_DT] ,[PA_END_DT] ,[CEP_PAY_CD_ID] ,[CEP_ENROLL_CD_ID] ,[CEP_ENROLL_EXCL_DESC] ,[NOVEL_STATUS_IND] ,[FIRST_NOVEL_MNTH] ,[SOURCE] ,[Is_Archived]) SELECT [CODE],[CODE_TYPE],[CODE_END_DT] ,[GENERIC_NAME] ,[TRADE_NAME] ,[CKPT_INHIB_IND] ,[ANTI_EMETIC_IND] ,[CODE_EFF_DT] ,[NHNR_CANCER_THERAPY] ,[CODE_CATEGORY_ID] ,[ASP_CATEGORY_ID] ,[DRUG_ADM_MODE_ID] ,[PA_DRUGS_ID] ,[PA_EFF_DT] ,[PA_END_DT] ,[CEP_PAY_CD_ID] ,[CEP_ENROLL_CD_ID] ,[CEP_ENROLL_EXCL_DESC] ,[NOVEL_STATUS_IND] ,[FIRST_NOVEL_MNTH] ,[SOURCE] ,0 as [Is_Archived] FROM [chemopx].[ChemotherapyPX_Tracking] WHERE [UPDATE_USER] = @username AND UPDATE_DT = @update_date AND [UPDATE_ACTION] = 'INSERT';
...
UPDATE Track
SET 
Track.[CODE_TYPE_PREVIOUS] = CASE WHEN ISNULL(Track.[CODE_TYPE],'') <> ISNULL(Chemo.[CODE_TYPE],'') OR Track.[CODE_TYPE_PREVIOUS] IS NULL THEN Chemo.[CODE_TYPE] ELSE Track.[CODE_TYPE] END,
Track.[CODE_END_DT_PREVIOUS] = CASE WHEN ISNULL(Track.[CODE_END_DT],'') <> ISNULL(Chemo.[CODE_END_DT],'') OR Track.[CODE_END_DT_PREVIOUS] IS NULL THEN Chemo.[CODE_END_DT] ELSE Track.[CODE_END_DT] END,
...
UPDATE Chemo
SET 
Chemo.[CODE_TYPE] = CASE WHEN  ISNULL(Track.[CODE_TYPE],'') <> ISNULL(Chemo.[CODE_TYPE],'') THEN Track.[CODE_TYPE] ELSE Chemo.[CODE_TYPE] END,
Chemo.[CODE_END_DT] = CASE WHEN  ISNULL(Track.[CODE_END_DT],'') <> ISNULL(Chemo.[CODE_END_DT],'') THEN Track.[CODE_END_DT] ELSE Chemo.[CODE_END_DT] END,
