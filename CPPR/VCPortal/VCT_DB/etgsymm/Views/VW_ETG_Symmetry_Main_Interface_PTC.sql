CREATE VIEW [etgsymm].[VW_ETG_Symmetry_Main_Interface_PTC]
	AS 
...
FROM  (SELECT * FROM etgsymm.ETG_Fact_Symmetry WHERE Symmetry_Version = (SELECT max(Symmetry_Version) FROM etgsymm.ETG_Fact_Symmetry)) AS f 
LEFT OUTER JOIN (SELECT * FROM etgsymm.ETG_Fact_Symmetry WHERE Symmetry_Version = (SELECT max(Symmetry_Version) FROM etgsymm.ETG_Fact_Symmetry WHERE Symmetry_Version < (SELECT MAX(Symmetry_Version) FROM etgsymm.ETG_Fact_Symmetry))) AS fp ON f.ETG_Base_Class = fp.ETG_Base_Class AND f.Premium_Specialty_id  = fp.Premium_Specialty_id 
