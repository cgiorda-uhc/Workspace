CREATE PROCEDURE [etgsymm].[sp_ETG_Symmetry_Adhoc]
	@version float = NULL
...
FROM  (
SELECT * FROM etgsymm.ETG_Fact_Symmetry WHERE PD_Version = 

(SELECT CASE WHEN @version IS NULL THEN max(PD_Version) ELSE @version END FROM etgsymm.ETG_Fact_Symmetry)

) AS f 
LEFT OUTER JOIN (SELECT * FROM etgsymm.ETG_Fact_Symmetry WHERE PD_Version = 
(
SELECT max(PD_Version) FROM etgsymm.ETG_Fact_Symmetry WHERE PD_Version 
< 
(SELECT CASE WHEN @version IS NULL THEN max(PD_Version) ELSE @version END FROM etgsymm.ETG_Fact_Symmetry)
)
) AS fp ON f.ETG_Base_Class = fp.ETG_Base_Class AND f.Premium_Specialty_id  = fp.Premium_Specialty_id 
