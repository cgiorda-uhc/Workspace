...
public static class EDCAdhoc_Calls
{
    private static readonly Serilog.ILogger _log = Serilog.Log.ForContext(typeof(EDCAdhoc_Calls));


    //STATIC EXTENSION FUNCTION ACT AS CONSTRUCTOR 
    public static void ConfigureEDCAdhocApi(this WebApplication app)
    {
        ...
        app.MapPost(pattern: "/edcadhoc", async ([FromBody] MHP_EI_Parameters param, IMHPUniverse_Repo repo, CancellationToken token) =>
        {
            try
            {
                ////RETURN HTTP 200
