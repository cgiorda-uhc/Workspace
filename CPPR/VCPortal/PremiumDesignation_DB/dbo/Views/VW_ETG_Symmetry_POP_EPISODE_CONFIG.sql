CREATE VIEW [dbo].[VW_ETG_Symmetry_POP_EPISODE_CONFIG] as
SELECT 
...
CASE WHEN f.[Has_NRX] = 1 THEN 'Y' ELSE  'N'END as  [NRx],
	...
	lob.title as Risk_Model,
	 ...
	 f.[EC_Treatment_Indicator] as [Current_EC_Treatment_Indicator]
	
FROM  dbo.ETG_Fact_Symmetry AS f 

INNER JOIN (SELECT ETG_Base_Class,Premium_Specialty_id ,'Commercial'as title  FROM dbo.ETG_Fact_Symmetry  UNION SELECT ETG_Base_Class,Premium_Specialty_id ,'Medicare'as title  FROM dbo.ETG_Fact_Symmetry UNION SELECT ETG_Base_Class,Premium_Specialty_id ,'Medicaid'as title  FROM dbo.ETG_Fact_Symmetry ) AS lob ON f.ETG_Base_Class = lob.ETG_Base_Class AND   f.Premium_Specialty_id = lob.Premium_Specialty_id 
...
AND f.[Data_Date] = (SELECT max(data_Date) FROM dbo.ETG_Fact_Symmetry)
