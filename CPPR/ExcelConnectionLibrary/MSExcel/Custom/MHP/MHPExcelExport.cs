...
namespace FileParsingLibrary.MSExcel.Custom.MHP
{
    public static class MHPExcelExport
    {
        ...
        public static async Task<byte[]> ExportEIToExcel(List<MHP_EI_Model> mhp_results, List<MHPUniverseDetails_Model> mhp_details, Func<string> getterStatus, Action<string> setterStatus, CancellationToken token)
        {
            ...
            foreach (MHPUniverseDetails_Model mhp in mhp_details)
            {
                if (token.IsCancellationRequested)
                {
                    break;
                ...
                if (lastEntity != mhp.LEG_ENTY_NBR)
                {
                    ...
                    range = wsSource.Range(wsSource.Cell(1, 1).Address, wsSource.Cell(1, typeof(MHPUniverseDetails_Model).GetProperties().Length).Address);
                ...
                wsSource.Cell("Q" + rowCnt).SetValue(mhp.Procedure_Code_Description.ToString());
                wsSource.Cell("R" + rowCnt).SetValue(mhp.Primary_Procedure_Code_Req.ToString());
                wsSource.Cell("S" + rowCnt).SetValue(mhp.Primary_Diagnosis_Code.ToString());
                wsSource.Cell("T" + rowCnt).SetValue(mhp.CUST_SEG_NBR.ToString());
                wsSource.Cell("U" + rowCnt).SetValue(mhp.CUST_SEG_NM.ToString());
