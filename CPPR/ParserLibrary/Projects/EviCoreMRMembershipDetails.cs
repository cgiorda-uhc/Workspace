...
public class EviCoreMRMembershipDetails : IEviCoreMRMembershipDetails
{
    private readonly IRelationalDataAccess _db;
    private readonly IEviCoreMRMembershipDetailsConfig? _config;
    private string _stagingArea;

    public EviCoreMRMembershipDetails(IConfiguration config, IRelationalDataAccess db)
    {
        //INJECT DB SOURCES
        _db = db;
    ...
    public async Task<long> LoadEviCoreMRMembershipDetails()
    {
        //CHECK FOR CONFIG
        if (_config == null)
        {
            Log.Error($"No Config found for EviCoreMRMembershipDetails data load");
        ...
        try
        {
            Log.Information($"Retrieving latest data from EviCoreMRMembershipDetails...");
            ...
            if (newFiles.Count == 0)
            {
                Log.Information($"No results found for EviCoreMRMembershipDetails. Will try again next time");
                stopwatch.Stop();
            ...
            foreach (var file in workingFiles)
            {
                ...
                var filename = Path.GetFileName(file);
                //LOOP EACH SHEET AND ADD TO EviCoreMRMembershipDetails
                foreach (var sheet in sheet_names)
                {
                    Log.Information($"Processing " + filename + " sheet:" + sheet);

                    var evi = closed_xml.ImportExcel<EviCoreMRMembershipDetailsModel>(file, sheet, config_sheet.ColumnRange, config_sheet.StartingDataRow, nullCheck: config_sheet.ColumnToValidate);
                    foreach (var e in evi)
                    {

                        //NOT IN SHEET
                        e.report_type = "COSMOS";
                        e.file_month = _month;
                        e.file_year = _year;
                    ...
                    //BULK LOAD CURRENT List<EviCoreMRMembershipDetailsModel> INTO DB
                    string[] columns = typeof(EviCoreMRMembershipDetailsModel).GetProperties().Select(p => p.Name).ToArray();
                    Log.Information($"Saving contents of " + filename + " sheet:" + sheet + " to database");
                    await _db.BulkSave<EviCoreMRMembershipDetailsModel>(sql.ConnectionString, _destination, evi, columns);


                    sql = _config.SQLLists.Where(x => x.Name == "Summary").FirstOrDefault();
                    Log.Information($"Refreshing EviCoreMRMembershipSummary");
        ...
        finally
        {
            if (blUpdated)
            {
                ...
                Log.Information($"EviCoreMRMembershipDetails process completed. Sending email to: " + email.EmailTo);
            ...
            else
            {
                ...
                Log.Information($"EviCoreMRMembershipDetails no new data found.");
            }
    ...
    private List<string> getNewFiles(List<FileExcelConfig> fileList, FileDateModel fdate)
    {
        ...
        foreach (var file in fileList)
        {
            ...
            Log.Information($"Searching for " + file.FilePath + "\\" + fdate.file_year + "\\" + file.FileName + "...");
            //var list = Directory.GetFiles(file.FilePath + "\\" + fdate.file_year, file.FileName, SearchOption.AllDirectories);
            var list = Directory.GetFiles(file.FilePath + "\\", file.FileName, SearchOption.AllDirectories);
 
            foreach (var f in list)
            {
                if(f.ToLower().Contains("_summary."))
                {
                    continue;
                }
        ...
        var list = new List<KeyValuePair<string, string>>
            {
                ...
                new KeyValuePair<string, string>("MemberCount","MemberCount")
            };
    ...
    private IEviCoreMRMembershipDetailsConfig prepareConfig(IConfiguration config)
    {


        var project = "EviCoreMRMembershipDetails";
        ...
        ///EXTRACT IConfiguration INTO EviCoreMRMembershipDetailsConfig
        var cfg = config.GetSection(section).Get<List<EviCoreMRMembershipDetailsConfig>>();
        IEviCoreMRMembershipDetailsConfig nc = new EviCoreMRMembershipDetailsConfig();
        if (cfg == null)
        {
            Log.Error($"No Config found for EviCoreMRMembershipDetails");
            throw new OperationCanceledException();
        }
