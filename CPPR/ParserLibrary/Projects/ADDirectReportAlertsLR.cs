        ...
        private List<ADDirectReportAlertsLRModel> getAllADDirectReport(string path, string domain, string username, string password, List<string> managers)
        {
            ...
            foreach (string mng in managers)
            {
                ...
                foreach (var u in _users)
                {
                    ...
                    _usersFinal.Add(new ADDirectReportAlertsLRModel {userid=u.LoginName, email = u.EmailAddress, employee_name = u.FullName, manager_name = ( m == null?  "Not Listed" : m.FullName)});
                }
            }
            return _usersFinal;
        }
