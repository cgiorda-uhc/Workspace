...
namespace ConsoleLibraryTesting
{
    public class AdHoc
    {
        ...
        public async Task generateTATReportsAsync()
        {
            ...
            sbSQL.Append("DECLARE @CURRENT_DATE INT; SELECT @CURRENT_DATE= year(max(file_date)) FROM stg.EviCore_TAT;");
            sbSQL.Append("select sort_id,LOB, rpt_Modality,metric ,isnull(sum(Penalty_SLA), 0) YTD_Penalty ,isnull(sum(case when month(tmp.file_date) = 1 then tmp.Penalty_SLA end), 0) Jan ,isnull(sum(case when month(tmp.file_date) = 2 then tmp.Penalty_SLA end), 0) Feb ,isnull(sum(case when month(tmp.file_date) = 3 then tmp.Penalty_SLA end), 0) Mar ,isnull(sum(case when month(tmp.file_date) = 4 then tmp.Penalty_SLA end), 0) Apr ,isnull(sum(case when month(tmp.file_date) = 5 then tmp.Penalty_SLA end), 0) May ,isnull(sum(case when month(tmp.file_date) = 6 then tmp.Penalty_SLA end), 0) Jun ,isnull(sum(case when month(tmp.file_date) = 7 then tmp.Penalty_SLA end), 0) Jul ,isnull(sum(case when month(tmp.file_date) = 8 then tmp.Penalty_SLA end), 0) Aug ,isnull(sum(case when month(tmp.file_date) = 9 then tmp.Penalty_SLA end), 0) Sep ,isnull(sum(case when month(tmp.file_date) = 10 then tmp.Penalty_SLA end), 0) Oct ,isnull(sum(case when month(tmp.file_date) = 11 then tmp.Penalty_SLA end), 0) Nov ,isnull(sum(case when month(tmp.file_date) = 12 then tmp.Penalty_SLA end), 0) Dec from (Select 1 as sort_id,'ASA (30 second metric)' as metric, t.file_date,ref.LOB, ref.rpt_Modality, Avg_Speed_Answer, s.SLA, CASE WHEN t.Avg_Speed_Answer > s.SLA THEN s.Penalty_SLA else 0 end as Penalty_SLA from (select distinct lob,rpt_Modality from stg.EviCore_YTDMetrics where rpt_Modality<>'' and lob<>'Empire') as ref left join (Select file_date,lob,rpt_Modality, Avg_Speed_Answer, round(Abandoned_Percent,3) as Abandoned_Pct from stg.EviCore_YTDMetrics where year(file_date)=@CURRENT_DATE and rpt_Modality<>'' and lob not in('Empire','E&I') UNION ALL Select file_date,lob,rpt_Modality, CAST(ROUND(sum(Total_Calls * Avg_Speed_Answer)/sum(Total_Calls),2) as decimal(5,0)) as Avg_Speed_Answer, CAST(ROUND(sum(Abandoned_Calls)/sum(Total_Calls),3) as decimal(5,4)) as Abandoned_Pct from stg.EviCore_YTDMetrics where year(file_date)=@CURRENT_DATE and lob='E&I' and rpt_Modality<>'' group by file_date,lob,rpt_Modality) as t on ref.lob=t.lob and ref.rpt_modality=t.rpt_modality left JOIN (select * from stg.SLA_Lookup WHERE Metric='Average Speed to Answer (ASA)') as s ON ref.lob = s.LOB AND ref.rpt_Modality = s.Modality ) as tmp group by sort_id,metric, LOB, rpt_Modality ");
            sbSQL.Append("UNION ALL ");
            sbSQL.Append("select sort_id,LOB, rpt_Modality,metric ,isnull(sum(Penalty_SLA), 0) YTD_Penalty ,isnull(sum(case when month(tmp.file_date) = 1 then tmp.Penalty_SLA end), 0) Jan ,isnull(sum(case when month(tmp.file_date) = 2 then tmp.Penalty_SLA end), 0) Feb ,isnull(sum(case when month(tmp.file_date) = 3 then tmp.Penalty_SLA end), 0) Mar ,isnull(sum(case when month(tmp.file_date) = 4 then tmp.Penalty_SLA end), 0) Apr ,isnull(sum(case when month(tmp.file_date) = 5 then tmp.Penalty_SLA end), 0) May ,isnull(sum(case when month(tmp.file_date) = 6 then tmp.Penalty_SLA end), 0) Jun ,isnull(sum(case when month(tmp.file_date) = 7 then tmp.Penalty_SLA end), 0) Jul ,isnull(sum(case when month(tmp.file_date) = 8 then tmp.Penalty_SLA end), 0) Aug ,isnull(sum(case when month(tmp.file_date) = 9 then tmp.Penalty_SLA end), 0) Sep ,isnull(sum(case when month(tmp.file_date) = 10 then tmp.Penalty_SLA end), 0) Oct ,isnull(sum(case when month(tmp.file_date) = 11 then tmp.Penalty_SLA end), 0) Nov ,isnull(sum(case when month(tmp.file_date) = 12 then tmp.Penalty_SLA end), 0) Dec from (Select 2 as sort_id,'Telephone Abandonment rate (<=3.00%)' as metric, t.file_date,ref.LOB, ref.rpt_Modality, Abandoned_Pct, s.SLA, CASE WHEN t.Abandoned_Pct > s.SLA THEN s.Penalty_SLA else 0 end as Penalty_SLA from (select distinct lob,rpt_Modality from stg.EviCore_YTDMetrics where rpt_Modality<>'' and lob<>'Empire') as ref left join (Select file_date,lob,rpt_Modality, Avg_Speed_Answer, round(Abandoned_Percent,3) as Abandoned_Pct from stg.EviCore_YTDMetrics where year(file_date)=@CURRENT_DATE and rpt_Modality<>'' and lob not in('Empire','E&I') UNION ALL Select file_date,lob,rpt_Modality, CAST(ROUND(sum(Total_Calls * Avg_Speed_Answer)/sum(Total_Calls),2) as decimal(5,0)) as Avg_Speed_Answer, CAST(ROUND(sum(Abandoned_Calls)/sum(Total_Calls),3) as decimal(5,4)) as Abandoned_Pct from stg.EviCore_YTDMetrics where year(file_date)=@CURRENT_DATE and lob='E&I' and rpt_Modality<>'' group by file_date,lob,rpt_Modality) as t on ref.lob=t.lob and ref.rpt_modality=t.rpt_modality left JOIN (select * from stg.SLA_Lookup WHERE Metric='Abandonment Rate') as s ON ref.lob = s.LOB AND ref.rpt_Modality = s.Modality ) as tmp group by sort_id,metric, LOB, rpt_Modality ");
            sbSQL.Append("UNION ALL ");
            sbSQL.Append("select sort_id,LOB, rpt_Modality,metric ,isnull(sum(Penalty_SLA), 0) YTD_Penalty ,isnull(sum(case when month(tmp.file_date) = 1 then tmp.Penalty_SLA end), 0) Jan ,isnull(sum(case when month(tmp.file_date) = 2 then tmp.Penalty_SLA end), 0) Feb ,isnull(sum(case when month(tmp.file_date) = 3 then tmp.Penalty_SLA end), 0) Mar ,isnull(sum(case when month(tmp.file_date) = 4 then tmp.Penalty_SLA end), 0) Apr ,isnull(sum(case when month(tmp.file_date) = 5 then tmp.Penalty_SLA end), 0) May ,isnull(sum(case when month(tmp.file_date) = 6 then tmp.Penalty_SLA end), 0) Jun ,isnull(sum(case when month(tmp.file_date) = 7 then tmp.Penalty_SLA end), 0) Jul ,isnull(sum(case when month(tmp.file_date) = 8 then tmp.Penalty_SLA end), 0) Aug ,isnull(sum(case when month(tmp.file_date) = 9 then tmp.Penalty_SLA end), 0) Sep ,isnull(sum(case when month(tmp.file_date) = 10 then tmp.Penalty_SLA end), 0) Oct ,isnull(sum(case when month(tmp.file_date) = 11 then tmp.Penalty_SLA end), 0) Nov ,isnull(sum(case when month(tmp.file_date) = 12 then tmp.Penalty_SLA end), 0) Dec from (Select 3 as sort_id,'Standard PA TAT (100% metric)' as metric,e.file_date, case when t.lob like '%E&I%' then 'E&I' else t.lob end as lob,t.rpt_Modality, cast(num/denom as decimal(6,4)) as pct,s.SLA, CASE when cast(num/denom as decimal(6,4)) < s.SLA THEN s.Penalty_SLA else 0 end as Penalty_SLA from (select distinct report_type,rpt_modality,lob FROM stg.EviCore_TAT where report_type = 'Routine TAT' and NOT (lob in('EXCHANGE','Empire','E&I_Notif') or lob is null)) as t LEFT JOIN (Select file_date,report_type,LOB,rpt_modality, cast(sum(LessEqual_2_BUS_Days) as decimal) as num, cast(sum(Total_Authorizations_Notifications)as decimal) as denom from stg.EviCore_TAT as e where year(file_date)= @CURRENT_DATE and report_type = 'Routine TAT' group by file_date,report_type,lob, rpt_Modality ) as e on t.rpt_modality=e.rpt_modality and t.lob=e.lob and t.report_type=e.report_type LEFT JOIN stg.SLA_Lookup as s ON t.LOB = s.LOB AND t.rpt_Modality = s.Modality AND t.report_type = s.Metric ) as tmp group by sort_id,metric, LOB, rpt_Modality ");
            sbSQL.Append("UNION ALL ");
            sbSQL.Append("select sort_id,LOB, rpt_Modality,metric ,isnull(sum(Penalty_SLA), 0) YTD_Penalty ,isnull(sum(case when month(tmp.file_date) = 1 then tmp.Penalty_SLA end), 0) Jan ,isnull(sum(case when month(tmp.file_date) = 2 then tmp.Penalty_SLA end), 0) Feb ,isnull(sum(case when month(tmp.file_date) = 3 then tmp.Penalty_SLA end), 0) Mar ,isnull(sum(case when month(tmp.file_date) = 4 then tmp.Penalty_SLA end), 0) Apr ,isnull(sum(case when month(tmp.file_date) = 5 then tmp.Penalty_SLA end), 0) May ,isnull(sum(case when month(tmp.file_date) = 6 then tmp.Penalty_SLA end), 0) Jun ,isnull(sum(case when month(tmp.file_date) = 7 then tmp.Penalty_SLA end), 0) Jul ,isnull(sum(case when month(tmp.file_date) = 8 then tmp.Penalty_SLA end), 0) Aug ,isnull(sum(case when month(tmp.file_date) = 9 then tmp.Penalty_SLA end), 0) Sep ,isnull(sum(case when month(tmp.file_date) = 10 then tmp.Penalty_SLA end), 0) Oct ,isnull(sum(case when month(tmp.file_date) = 11 then tmp.Penalty_SLA end), 0) Nov ,isnull(sum(case when month(tmp.file_date) = 12 then tmp.Penalty_SLA end), 0) Dec from (Select 4 as sort_id,'Urgent TAT (100% metric)' as metric,e.file_date, case when t.lob like '%E&I%' then 'E&I' else t.lob end as lob,t.rpt_Modality, cast(num/denom as decimal(6,4)) as pct,s.SLA, CASE when cast(num/denom as decimal(6,4)) < s.SLA THEN s.Penalty_SLA else 0 end as Penalty_SLA from (select distinct report_type,rpt_modality,lob FROM stg.EviCore_TAT where report_type = 'Urgent TAT' and NOT (lob in('EXCHANGE','Empire','E&I_Notif') or lob is null)) as t LEFT JOIN (Select file_date,report_type,LOB,rpt_modality, cast(sum(Less_State_TAT_Requirements) as decimal) as num, cast(sum(Total_Authorizations_Notifications)as decimal) as denom from stg.EviCore_TAT as e where year(file_date)=@CURRENT_DATE and report_type = 'Urgent TAT' group by file_date,report_type,lob, rpt_Modality ) as e on t.rpt_modality=e.rpt_modality and t.lob=e.lob and t.report_type=e.report_type LEFT JOIN stg.SLA_Lookup as s ON t.LOB = s.LOB AND t.rpt_Modality = s.Modality AND t.report_type = s.Metric ) as tmp group by sort_id,metric, LOB, rpt_Modality ");
            sbSQL.Append("UNION ALL ");
            sbSQL.Append("select sort_id,lob, rpt_Modality,metric ,isnull(sum(Penalty_SLA), 0) YTD_Penalty ,isnull(sum(case when month(tmp.file_date) = 1 then tmp.Penalty_SLA end), 0) Jan ,isnull(sum(case when month(tmp.file_date) = 2 then tmp.Penalty_SLA end), 0) Feb ,isnull(sum(case when month(tmp.file_date) = 3 then tmp.Penalty_SLA end), 0) Mar ,isnull(sum(case when month(tmp.file_date) = 4 then tmp.Penalty_SLA end), 0) Apr ,isnull(sum(case when month(tmp.file_date) = 5 then tmp.Penalty_SLA end), 0) May ,isnull(sum(case when month(tmp.file_date) = 6 then tmp.Penalty_SLA end), 0) Jun ,isnull(sum(case when month(tmp.file_date) = 7 then tmp.Penalty_SLA end), 0) Jul ,isnull(sum(case when month(tmp.file_date) = 8 then tmp.Penalty_SLA end), 0) Aug ,isnull(sum(case when month(tmp.file_date) = 9 then tmp.Penalty_SLA end), 0) Sep ,isnull(sum(case when month(tmp.file_date) = 10 then tmp.Penalty_SLA end), 0) Oct ,isnull(sum(case when month(tmp.file_date) = 11 then tmp.Penalty_SLA end), 0) Nov ,isnull(sum(case when month(tmp.file_date) = 12 then tmp.Penalty_SLA end), 0) Dec from (Select 5 as sort_id,'Report Timeliness (monthly)' as metric,e.file_date, t.lob,t.rpt_Modality, delivery_date, CASE when e.delivery_date > datefromparts ( year(e.file_date), month(e.file_date), ( CASE WHEN dbo.fn_IsHoliday(datefromparts(year(e.file_date), month(e.file_date), '15')) = 1 AND DATEPART(DW,datefromparts(year(e.file_date), month(e.file_date), '15')) = 2 THEN '16' ELSE CASE WHEN DATEPART(DW,datefromparts(year(e.file_date), month(e.file_date), '15')) = 7 THEN CASE WHEN dbo.fn_IsHoliday(datefromparts(year(e.file_date), month(e.file_date), '17')) = 1 THEN '18' ELSE '17' END ELSE CASE WHEN DATEPART(DW,datefromparts(year(e.file_date), month(e.file_date), '15')) = 1 THEN CASE WHEN dbo.fn_IsHoliday(datefromparts(year(e.file_date), month(e.file_date), '16')) = 1 THEN '17' ELSE '16' END ELSE '15' END END END ))THEN s.Penalty_SLA else 0 end as Penalty_SLA from (select distinct lob,rpt_Modality from stg.EviCore_YTDMetrics where rpt_Modality<>'' and lob<>'Empire') as t LEFT JOIN (Select file_date,report_type,LOB,rpt_modality,delivery_date from stg.EviCore_YTDMetrics as e where year(file_date)=@CURRENT_DATE and lob<>'Empire' group by file_date,report_type,lob, rpt_Modality, delivery_date ) as e on t.rpt_modality=e.rpt_modality and t.lob=e.lob LEFT JOIN (SELECT * FROM stg.SLA_Lookup WHERE Metric = 'Report timeliness') as s ON t.LOB = s.LOB AND t.rpt_Modality = s.Modality ) as tmp group by sort_id,metric, LOB, rpt_Modality ");
            sbSQL.Append("order by rpt_modality desc, lob,sort_id ");
            var tsum = await db_sql.LoadData<TAT_Summary_Model>(connectionString: ConnectionStringMSSQL, strSQL);
            export.Add(new ExcelExport() { ExportList = asa.ToList<object>(), SheetName = strSheetName });
